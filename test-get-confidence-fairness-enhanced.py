...
def test_one_dataset(model, data_loader):
    ...
    for i, data_dict in tqdm(enumerate(data_loader)):
        ...
        predictions['cls'] = torch.sigmoid(predictions['cls'])
        confidences.append(predictions['cls'].cpu().detach().numpy())
...
def test_one_dataset_cvpr2024(model, data_loader):
    confidences = []
    for i, data_dict in tqdm(enumerate(data_loader)):
        # get data
        data, label, mask, landmark = data_dict['image'], data_dict['label'], data_dict['mask'], data_dict['landmark']
...
def test_epoch(model, test_data_loaders, model_name):
    ...
    for key in keys:
        ...
        if model_name == "fair_df_detector":
            confidences = test_one_dataset_cvpr2024(model, test_data_loaders[key])
        else:
